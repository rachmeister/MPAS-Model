...
!             enddo
            ...
            Do k = disturbance_level_ind_b, disturbance_level_ind_t
             randmean =  2.0_wp * disturbance_amplitude * &
                          ( random_function( iran ) - 0.5_wp )
       ...
       do k = disturbance_level_ind_b, disturbance_level_ind_t
          ...
          DO  i = nxlg, nxrg 
              ...
              dist1(k,j,i) = float (int(randmean2 * 1.0E10_wp + 0.5_wp)) / 1.0E10_wp
              enddo
          enddo

       enddo

       print *, 'minmax again = ',minval(dist1),maxval(dist1)
       ...
       DO  i = dist_nxl(dist_range), dist_nxr(dist_range)
          DO  j = dist_nys(dist_range), dist_nyn(dist_range)
             ...
             DO  k = disturbance_level_ind_b, disturbance_level_ind_t
                ...
                randomnumber(k,j,i) = 3.0_wp * disturbance_amplitude *                &
                               ( random_dummy - 0.5_wp )
                IF ( nxl <= i  .AND.  nxr >= i  .AND.  nys <= j  .AND.         &
                     nyn >= j )                                                &
                THEN
                   dist1(k,j,i) = randomnumber(k,j,i)
                ENDIF
             ENDDO
    ...
    ELSEIF ( random_generator == 'system-specific' )  THEN
       DO  i = dist_nxl(dist_range), dist_nxr(dist_range)
          DO  j = dist_nys(dist_range), dist_nyn(dist_range)
             DO  k = disturbance_level_ind_b, disturbance_level_ind_t
                CALL RANDOM_NUMBER( randomnumber(k,j,i) )
                randomnumber(k,j,i) = 3.0_wp * disturbance_amplitude *                &
                                ( randomnumber(k,j,i) - 0.5_wp )
...
!                            + dist1(k,j+1,i) + dist1(k,j-1,i)                  &
!                            + 4.0_wp*dist1(k,j,i)) / 8.0_wp
!          ENDDO
...
!                            + dist2(k,j+1,i) + 4.0_wp * dist2(k,j,i)                            &
!                            ) / 8.0_wp
!          ENDDO
!       ENDDO
!    ENDDO
...
!-- Random perturbation is added to the array to be disturbed.
    DO  i = nxlg, nxrg
       DO  j = nysg, nyng
!          DO  k = nzb+1, nzt
!             field(k,j,i) = field(k,j,i) - (meanCurrent(k) - meanTarget(k)) / 3600.0_wp
!          ENDDO
