...
!------------------------------------------------------------------------------!
 SUBROUTINE disturb_field( var_char, dist1, field )
    ...
    INTEGER(iwp) :: k1,k2,i1,i2,j1,j2

    REAL(wp) ::  randmean2, randomnumber  !<
...
!-- to store the disturbance
    ...
    print *, disturbance_level_ind_b, disturbance_level_ind_t
!
!-- Create the random perturbation and store it on temporary array
    IF ( random_generator == 'numerical-recipes' )  THEN
       DO  i = nxlg,nxrg !dist_nxl(dist_range), dist_nxr(dist_range)
          DO  j = nysg,nyng !dist_nys(dist_range), dist_nyn(dist_range)
             DO  k = disturbance_level_ind_b, disturbance_level_ind_t
                ...
                IF ( nxl <= i  .AND.  nxr >= i  .AND.  nys <= j  .AND.         &
                     nyn >= j )                                                &
                THEN
               !    randmean2 = float (int(randomnumber * 1.0E8_wp + 0.5_wp)) / 1.0E10_wp
       ...
       k1 = disturbance_level_ind_b
       k2 = disturbance_level_ind_t
       i1 = nxlg
       i2 = nxrg
       j1 = nysg
       j2 = nyng
